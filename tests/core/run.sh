#!/bin/bash
# run.sh [pasriscv-src-dir]: builds PrivProbe (from ../priv) against PasRISCV.pas, assembles every
# test *.S, runs it in the interpreter (single step) and with the JIT (Machine.Run), and checks the
# registers named in the "# expect:" line of the test (a subset of s0 to s10 and a0 to a7, as 16
# uppercase hex digits) in both runs. A test with a "# jitonly" line only runs with the JIT.
#
...
WORK=${WORK:-$HERE/out}
MARCH=rv64gcvh_zfh_zfa_zfbfmin_svinval_zicfiss_zicfilp_zimop_zcmop_zacas_zabha_zbc_zicboz_zvfh_zvksh
FAILED=0
...
if ! "$FPC" -Mdelphi -O2 -Fu"$SRC" -Fu"$EXTDIR/pasmp/src" -Fu"$EXTDIR/rnl/src" -Fu"$EXTDIR/pasterm/src" \
             -FU"$WORK/units" -FE"$WORK" "$HERE/../priv/PrivProbe.dpr" > "$WORK/build.log" 2>&1; then
...
for S in "$HERE"/*.S; do
  ...
  if ! "${CROSS}gcc" -nostdlib -nostartfiles -march=$MARCH -mabi=lp64d -Wl,--build-id=none -Wl,-Ttext=0x80000000 \
       -I"$HERE" -I"$HERE/../priv" -I"$HERE/../hext" -o "$WORK/$NAME.elf" "$S" > "$WORK/$NAME.as.log" 2>&1 ||
