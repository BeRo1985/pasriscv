#!/bin/bash
# run.sh [pasriscv-src-dir]: builds HProbe (../hext) against PasRISCV.pas, assembles every test *.S and
# checks the registers named in the "# expect:" line of the test (a subset of s0 to s10 and a0 to
# a7, as 16 uppercase hex digits). A test with a "# aia" line runs with the AIA enabled.
#
...
echo "== build HProbe against $SRC/PasRISCV.pas"
if ! "$FPC" -Mdelphi -O2 -Fu"$SRC" -Fu"$EXTDIR/pasmp/src" -Fu"$EXTDIR/rnl/src" -Fu"$EXTDIR/pasterm/src" \
             -FU"$WORK/units" -FE"$WORK" "$HERE/../hext/HProbe.dpr" > "$WORK/build.log" 2>&1; then
...
for S in "$HERE"/*.S; do
  ...
  OPTION=""
  if grep -q '^# aia$' "$S"; then
    OPTION="aia"
  fi
  if ! "${CROSS}gcc" -nostdlib -nostartfiles -march=rv64gch_svinval -mabi=lp64d -Wl,--build-id=none -Wl,-Ttext=0x80000000 \
  ...
  GOT=$(timeout 30 "$WORK/HProbe" "$WORK/$NAME.bin" $OPTION)
  RC=$?
  if [ $RC -ne 0 ]; then
