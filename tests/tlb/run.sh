#!/bin/bash
# run.sh [pasriscv-src-dir]: builds PMPProbe (from ../pmp) against PasRISCV.pas, assembles every test *.S and
...
if ! "$FPC" -Mdelphi -O2 -Fu"$SRC" -Fu"$EXTDIR/pasmp/src" -Fu"$EXTDIR/rnl/src" -Fu"$EXTDIR/pasterm/src" \
             -FU"$WORK/units" -FE"$WORK" "$HERE/../pmp/PMPProbe.dpr" > "$WORK/build.log" 2>&1; then
