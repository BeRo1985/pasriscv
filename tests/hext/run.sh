#!/bin/bash
# run.sh [pasriscv-src-dir]: builds HProbe against PasRISCV.pas, assembles every test *.S and
...
if ! "$FPC" -Mdelphi -O2 -Fu"$SRC" -Fu"$EXTDIR/pasmp/src" -Fu"$EXTDIR/rnl/src" -Fu"$EXTDIR/pasterm/src" \
             -FU"$WORK/units" -FE"$WORK" "$HERE/HProbe.dpr" > "$WORK/build.log" 2>&1; then
...
if [ $FAILED = 0 ]; then
  echo "== all H extension tests match"
else
  echo "== FAILED"
fi
exit $FAILED
